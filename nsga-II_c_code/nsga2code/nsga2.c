...
//    input(rep_ptr);
    ...
    //马尔可夫使用到的参数
    int solLength = popsize; //使用时需要对该变量进行赋值，该变量指个体数量，gens的长度对应的是目标的个数
    /* pv、alpha是用来保存马尔可夫网络相关参数的全局数组，通过它们可以使得每次变异之间产生联系 */
    double pv[solLength];
    double alpha[solLength + 1];

    for (int pi = 0; pi < solLength; pi++) {
        pv[pi] = 0.5;
    }
...
//            printf("nsga_f[%d] = %f\n", tj, old_pop_ptr->ind[ti].fitness[tj]);
//        }
//    }
...
//        for (int ti = 0; ti < popsize; ti++) {
//            for (int tj = 0; tj < nfunc; tj++) {
//                printf("nsga_mate_pop_ptr_f[%d] = %f\n", tj, mate_pop_ptr->ind[ti].fitness[tj]);
//                printf("nsga_old_pop_ptr_f[%d] = %f\n", tj, old_pop_ptr->ind[ti].fitness[tj]);
//            }
//        }
...
//      /*Real Cross-over*/
        ...
        if (nchrom > 0)
//	mutate(new_pop_ptr );
            markovMut(new_pop_ptr, i + 1, pv, alpha, solLength);
        /*Binary Mutation */
        ...
        for (j = 0; j < maxrank1; j++) {
            ...
            fprintf(rep2_ptr, " %d\t  %d\t\t %d\t %f\n", j + 1, old_pop_ptr->rankno[j], new_pop_ptr->rankno[j],
                    new_pop_ptr->rankrat[j]);
      ...
      at different ranks */
        ...
        if (i == gener - 1) {  // for the last generation
            old_pop_ptr = &(matepop);
            for (f = 0; f < popsize; f++) // for printing
            {
                ...
                if ((old_pop_ptr->ind_ptr->error <= 0.0) &&
                    (old_pop_ptr->ind_ptr->rank == 1))  // for all feasible solutions and non-dominated solutions
                {
                    for (l = 0; l < nfunc; l++)
